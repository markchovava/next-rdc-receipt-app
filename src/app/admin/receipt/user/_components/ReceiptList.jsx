...
import { receiptDeleteApiAction, receiptListByUserApiAction, receiptPaginationApiAction, receiptSearchByUserApiAction } from '@/actions/receiptActions';
...
export default function ReceiptList({ dbData, userData, machineUserData, districtsData }) {
    ...
    async function getSearchData() {
        ...
        try{
            const res = await receiptSearchByUserApiAction(search);
            receiptDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links?.prev,
                nextURL: res?.links?.next,
    ...
    async function getData() {
        try{
            const res = await receiptListByUserApiAction();
            receiptDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
                nextURL: res?.links.next,
