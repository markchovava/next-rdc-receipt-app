...
import { districtDeleteApiAction, districtListApiAction, districtPaginationApiAction, districtSearchApiAction } from '@/actions/districtActions';
...
export default function DistrictList({ dbData }) {
    const { districtState, districtDispatch } = AdminContextState();
    const [isPending, startTransition] = useTransition();
    const [search, setSearch] = useState('');
    const [isModal, setIsModal] = useState('');

    useEffect(() => {
        districtDispatch({type: 'ADD_DATA', payload: {
            items: dbData?.data,
    ...
    async function paginateHandler(url) {
        try{
            const res = await districtPaginationApiAction(url)
            districtDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
    ...
    async function getSearchData() {
        if(!search) {
          ...
          try{
            const res = await districtSearchApiAction(search);
            startTransition(() => res);
            districtDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
    ...
    async function getData() {
        try{
            const res = await districtListApiAction();
            districtDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
                nextURL: res?.links.next,
    ...
    async function deleteData(id) {
        try{
            const res = await districtDeleteApiAction(id);
            if(res.status == 1) {
                await getData();
    ...
    if(!districtState.items) { return <Loader /> }
  ...
  return (
    ...
    <section className="w-[100%] lg:overflow-hidden overflow-scroll">
        ...
        <div className='mx-auto w-[96%] text-lg py-2 flex items-center justify-start font-bold bg-slate-200 '>
            ...
            <div className='w-[35%] border-r border-white px-3 py-2'>AUTHOR</div>
            <div className='w-[15%] px-3 py-2'>ACTION</div>
        </div>
        {/* DETAILS */}
        { districtState?.items?.length > 0 ?
          districtState.items.map((i, key) => (
    ...
    <section className='mx-8 pt-8 pb-12 flex items-center justify-end gap-3'>
        {/* PREVIOUS */}
        {districtState.prevURL &&
        <button
            title={districtState.prevURL}
            onClick={() => paginateHandler(districtState?.prevURL)}
        ...
        {/* NEXT */}
        {districtState.nextURL &&
        <button 
            title={districtState.nextURL}
            onClick={() => paginateHandler(districtState?.nextURL)}
            className='group flex items-center justify-center gap-2 text-cyan-700 '>
            <span>Next</span>
